#-- paths modified from SimsToEstChronicRateBatchCodeV2_2.R on lionxh 30 Nov 2013. --#

#-- specify path to "root" --#
root.path <- "~/work/Kezia/Research/EcologyPapers/ClustersAssocations_V2/ClustersAssociations/"

#-- source in AssocMatVec function --#
# source(paste(root.path, "Code/DataCleaning/RelocationsToNetworks/StaticNetworkAssocMat.R", sep = ""))

#-- 30 Nov 2013 --#
#-- function to build static graph of ewe connections in a given population-summer --#
#'data.subsets = list of pop-year data subsets that contains information on each ewe's lamb's survival
#'edgeweight.min = minimum edgeweight necessary for a connection to be included in the graph.
